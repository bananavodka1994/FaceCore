...
bool TELE::IsAllowedToTeleport(Player * const player) const
{
    ...
    switch (m_flag)
    {
      ...
      case FLAG_REPUTATION:
        return player->GetReputationRank(m_data0) >= ReputationRank(m_data1);
...
void LoadNpcTele(void)
{
    ...
    if (result)
    {
        TC_LOG_ERROR(LOG_FILTER_GENERAL, "TSCR: Loading %s, %s and %s...", Table[0], Table[1], Table[2]);
        ...
        TC_LOG_ERROR(LOG_FILTER_GENERAL, "TSCR: >> Loaded %u npc_teleport.", nbDest);
    } else TC_LOG_ERROR(LOG_FILTER_GENERAL, "TSCR: WARNING >> Loaded 0 npc_teleport.");
}
